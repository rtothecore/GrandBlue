...
bool SeahorseLayer::initWithPlist(const char* plist, const char* frameName)
{
	...
	if(frm_marineLife = SpriteFrameCache::getInstance()->getSpriteFrameByName(frameName))
	{
		...
		for(int i = 1; i < 5; i++) 
		{
			String* strSeahorse = String::createWithFormat("seahorse%d.png", i);
			SpriteFrame* frame = SpriteFrameCache::getInstance()->getSpriteFrameByName( strSeahorse->getCString() );
...
SeahorseLayer* SeahorseLayer::clone() const
{
	SeahorseLayer* ret = SeahorseLayer::create();
...
void SeahorseLayer::playMarineLifeSound()
{
	switch(byePoint)
	{
		case 1:
		case 2:
			Sound::playSeahorseEffectWithType(1);
			break;
		case 3:
		default:
			Sound::playSeahorseEffectWithType(2);
			break;
	}
}