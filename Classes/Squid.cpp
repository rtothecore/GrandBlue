...
bool SquidLayer::initWithPlist(const char* plist, const char* frameName)
{
	...
	if(frm_marineLife = SpriteFrameCache::getInstance()->getSpriteFrameByName(frameName))
	{
		...
		for(int i = 1; i < 6; i++) 
		{
			String* strSquid = String::createWithFormat("squid%d.png", i);
			SpriteFrame* frame = SpriteFrameCache::getInstance()->getSpriteFrameByName( strSquid->getCString() );
...
SquidLayer* SquidLayer::clone() const
{
	SquidLayer* ret = SquidLayer::create();
...
void SquidLayer::playMarineLifeSound()
{
	switch(byePoint)
	{
		case 1:
		case 2:
			Sound::playSquidEffectRand();
			break;
		case 3:
		default:
			Sound::playSquidEffectWithType(3);
			break;
	}
}